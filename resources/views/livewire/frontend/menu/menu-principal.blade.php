    ...
    <nav class="navbar">
        ...
        <div :class="{ 'block': abiertoSidebar, 'block': !abiertoSidebar }" class="contenedor_menu_link">
            <div class="sidebar_logo">
                <img src="{{ asset('Inicio/imagenes/logo.png') }}" alt="" />
                <i x-on:click="cerrarSidebar" class="fa-solid fa-xmark"></i>
            </div>
            <!-- MENU-PRINCIPAL -->
                    ...
                    <div x-data="subMenu1" class="elementos_menu_principal">
                        ...
                        <div x-on:click="seleccionar({{ $key }})" class="menu_icono">
                            @if (count($menu['subMenu1']))
                                <a>{{ $menu['nombrePrincipal'] }}</a>
                                <i class="fa-solid fa-sort-down"></i>
                            @else
                                <a href={{ $menu['nombrePrincipalUrl'] }}>{{ $menu['nombrePrincipal'] }}</a>
                            @endif
                        </div>

                        <!--SubMenu1-->
                        <div x-show="seleccionado == {{ $key }}" class="submenu_1 submenu_1_activo">
                            @if (count($menu['subMenu1']))
                                @foreach ($menu['subMenu1'] as $keySub1 => $subMenu1)
                                    <div x-data="subMenu2" class="elementos_submenu_1">

                                        <!--SubMenu1 Nombres-->
                                        <div x-on:click="seleccionarSubMenu1({{ $keySub1 }})" class="menu_icono">
                                            @if (count($subMenu1['subMenu2']))
                                                <a>{{ $subMenu1['nombreSubMenu1'] }}</a>
                                                <i class="fa-solid fa-sort-down"></i>
                                            @else
                                                <a
                                        ...
                                        <!--SubMenu2-->
                                        <div x-show="seleccionadoSubMenu1 == {{ $keySub1 }}"
                                            class="submenu_2 submenu_2_activo">
                                            @if (count($subMenu1['subMenu2']))
                                                @foreach ($subMenu1['subMenu2'] as $keySub2 => $subMenu2)
                                                    <div x-data="subMenu3" class="elementos_submenu_2">
                                                        ...
                                                        <div x-show="seleccionadoSubMenu2 == {{ $keySub2 }}"
                                                            class="submenu_3 submenu_3_activo">
                                                            @if (count($subMenu2['subMenu3']))
                                                                @foreach ($subMenu2['subMenu3'] as $keySub3 => $subMenu3)
                                                                    <div x-data="subMenu4" class="elementos_submenu_3">
                                                                        ...
                                                                        <div x-show="seleccionadoSubMenu3 == {{ $keySub3 }}"
                                                                            class="submenu_4 submenu_4_activo">
                                                                            @if (count($subMenu3['subMenu4']))
