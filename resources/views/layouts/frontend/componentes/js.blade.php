<script src="{{ asset('frontend/inicio/js/slider-principal.js') }}"></script>
<script src="{{ asset('frontend/inicio/js/slider-producto.js') }}"></script>
<script src="{{ asset('frontend/inicio/js/slider-producto2.js') }}"></script>
<script src="{{ asset('frontend/inicio/js/slider-testimonio.js') }}"></script>
<script src="{{ asset('frontend/inicio/js/slider-iconos.js') }}"></script>
<script src="{{ asset('frontend/inicio/js/slider-metodo-pago.js') }}"></script>

<!-- Plugins -->
<script src="//cdn.jsdelivr.net/npm/sweetalert2@11"></script>
