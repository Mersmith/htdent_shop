        ...
        <div class="contenedor_2_elementos">
            ...
            <label class="label_principal">
                ...
                <select onchange="window.location.href=this.value;">
                    <option selected value="{{ route('cliente.orden.index') }}">Todos</option>
                    <option {{ $estado == 1 ? 'selected' : '' }}
                        value="{{ route('cliente.orden.index') . '?estado=1' }}">Falta
                        pagar</option>
                    <option {{ $estado == 2 ? 'selected' : '' }}
                        value="{{ route('cliente.orden.index') . '?estado=2' }}">
                        Alistado</option>
                    <option {{ $estado == 3 ? 'selected' : '' }}
                        value="{{ route('cliente.orden.index') . '?estado=3' }}">Enviado
                    </option>
                    <option {{ $estado == 4 ? 'selected' : '' }}
                        value="{{ route('cliente.orden.index') . '?estado=4' }}">
                        Entregado</option>
                    <option {{ $estado == 5 ? 'selected' : '' }}
                        value="{{ route('cliente.orden.index') . '?estado=5' }}">
                        Anulado</option>
                </select>
            </label>
        </div>
                    ...
                    @foreach ($ordenes as $orden)
                        <li style="border: 1px solid #ccc; border-radius: 5px; margin: 10px 0px;">
                            <a href="{{ $orden->estado == 1 ? route('cliente.orden.pagar', $orden) : route('cliente.orden.mostrar', $orden) }}"
                                ...
                                <span>
                                    <strong>Orden: N° 00000-{{ $orden->id }}</strong>
                                    <br>
                            ...
                            <div class="flex items-center py-2 px-4">
                                @php
                                    $productosCarrito = json_decode($orden->contenido, true);
